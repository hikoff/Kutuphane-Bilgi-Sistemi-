    ...
    public partial class kitapBul : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Baglanti.Open();
                    SqlCommand komut = new SqlCommand("SELECT kitapAd,sayfaSayisi,yayIsim,yazarIsim,turIsim from kitapBil INNER JOIN yayinList y1 ON KitapBil.yayId = y1.yayId INNER JOIN yazarList y2 ON KitapBil.yazarId = y2.yazarId INNER JOIN kitapTur k1 ON KitapBil.turId = k1.turId where yazarIsim=@yazarIsim and yayIsim=@yayIsim", Baglanti);
                    komut.Parameters.AddWithValue("@yayIsim", yay);
                    komut.Parameters.AddWithValue("@yazarIsim", yaz);
                    DataSet ds = new DataSet();
                    SqlDataAdapter da = new SqlDataAdapter(komut);
